...
- 可以直接使用模板
    - <a href='https://zhuanlan.zhihu.com/p/413978317'>21个Seaborn可视化案例</a>
    - <a href='https://blog.csdn.net/shelgi/article/details/103882760'>matplotlib+seaborn几种可视化图的模板</a>
    - <a href='https://blog.csdn.net/wzk4869/article/details/128746372'>Python统计分析可视化库Seaborn(相关性图, 变量分布图, 箱线图等等)</a>
<font color="#8968CD">
- Why import as sns?
...
### 模型选择的原则
- 一方面通过任务来决定模型, 另外, 还可以根据数据样本量以及特征的稀疏性来决定模型.
- 刚开始尝试一个基本的模型作为baseline, 进而再训练其他模型做对比, 最终选择泛化能力或性能比较好的模型
- scikit-learn主要的算法路径终点包括: 分类、回归、聚类、降维

### 数据准备

- 分割训练集和测试集, 设置随机种子以便结果能复现
- 当一个总体内部分层明显时, 分层抽样能够提高样本的代表性, 从而提高由样本推断总体的精确性; 分层抽样特别适用于既要对总体参数进行推断, 也要对各层的参数进行推断的情形
-  sklearn中切割数据集的方法为`sklearn.model_selection.train_test_split(*arrays, test_size=None, train_size=None, random_state=None, shuffle=True, stratify=None)`
    - `test_size`测试集比例, 默认0.25
    - `train_size`训练集比例
    - `random_state`
    - `shuffle`分割之前是否洗牌; 如果不洗牌则不允许分层.
    - `stratify`是否分层, 可以输入一个array
- 切割数据集的时候不用进行随机选取的情况
    - 数据集本身已经经过随机处理.
    - 处理的是时序数据.
    - 样本不均衡, 需要重采样等.
...
- LogisticRegression调参指南<a href='https://blog.csdn.net/OuDiShenmiss/article/details/105234824'>参考</a>
    - 正则化参数: penalty. {‘l1’,‘l2’, ‘elasticnet’, ‘none’}, default=’l2’
    - 损失函数优化器: solver. {‘liblinear’,‘lbfgs’, ‘sag’,‘newton-cg’, ‘saga’}, default=’lbfgs’
    - 分类方式选择参数: multi_class. {‘auto’, ‘ovr’, ‘multinomial’}, default=’auto’
    - 类型权重参数: class_weight. {dict or ‘balanced’}, default=None
    - C: 正则化系数λ的倒数. float类型, 默认为1.0. 必须是正浮点型数. 像SVM一样, 越小的数值表示越强的正则化.
- <a href='https://blog.csdn.net/Longtermevolution/article/details/100919721'>随机森林调参指南</a>


## 模型评估(具体内容见'第三章模型建立和评估---评价.ipynb')
